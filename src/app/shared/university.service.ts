...
export class UniversityService {
	...
	mapUnis(response:Response, token: string): University[]{
	   ...
	   // to do - if they figure out how to filter it by universities - I'll filter to only have the ones with the type id of 100418
	   //let unis = response.json().data.filter(university => university.type.id === 100418);
	   //return unis.map(uni => this.toUni(uni, token));
